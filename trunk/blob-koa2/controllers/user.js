...
class UserController {
    ...
    static async create(ctx) {
        ...
        if (user.username && user.password) {
            ...
            if (existUser) {
                ...
                // 创建用户
                let d=await userModel.create(user);
                console.log("d==",d);
    ...
    static async updateUser(ctx){
        ...
        if(!userInfo.id){
            ctx.body = statusCode.ERROR_403('缺少必要参数');
            return false;
        }
        const existUser = await userModel.findUserByName(userInfo.username)
        if(existUser){
            ctx.response.status = 403;
            ctx.body = statusCode.ERROR_403('用户名已经被使用')
        }else{
            let d= await userModel.updateUserById(userInfo);
            const userUpdate = await userModel.findUserById(userInfo.id)
            ctx.response.status = 200;
            ctx.body = statusCode.SUCCESS_200('更新成功',{
                user:userUpdate
            });    
        }
